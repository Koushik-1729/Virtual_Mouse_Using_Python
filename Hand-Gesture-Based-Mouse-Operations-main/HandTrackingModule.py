...
import math
import time
warnings.filterwarnings("ignore", category=UserWarning, message="Failed to get tag \"C60__PACKET\" index 0")
class HandDetector:
...
def main():
    ...
    # detector = HandDetector(detectionCon=0.6, maxHands=2, minTrackCon=0.5)
    ...
    while True:
        ...
        try:
            ...
            fps = 1 / (cTime - pTime)
            pTime = cTime
            k=cv2.putText(img, str(int(fps)), (10, 70), cv2.FONT_HERSHEY_PLAIN, 3, (255, 0, 255), 3)
        except:
            pass
